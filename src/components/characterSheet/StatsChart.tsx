...
const stats: {
  name: string;
  key: "strength" | "fortitude" | "speed" | "agility" | "dexterity" | "precision" | "intellect" | "sharpness" | "instinct" | "energy";
}[] = [
    {
      name: "Fuerza",
      key: "strength",
      ...
      <Grid container spacing={2}>
        {
          stats.map((stat) => (
            <>
              ...
              <Grid xs={3}>
                <FormInputText
                  stateValue={characterData[stat.key]?.base.toString()}
                  name={stat.key}
              ...
              <Grid xs={3}>
                <FormInputText
                  stateValue={characterData[stat.key]?.bonus.toString()}
                  name={`${stat.key}Bonus`}
                  control={props.control}
                  label={"Bonus"}
                />
              ...
              <Grid xs={3}>
                <Typography variant="h6">{characterData[stat.key]?.total.toString()}</Typography>
              </Grid>
            </>
          ))
        }
